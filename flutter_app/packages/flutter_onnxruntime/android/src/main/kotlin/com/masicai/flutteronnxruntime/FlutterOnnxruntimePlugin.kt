    ...
    override fun onMethodCall(
        ...
        when (call.method) {
            ...
            "runInference" -> {
                ...
                // Run inference off the Android main thread to prevent Flutter UI jank/freezes.
                inferenceExecutor.execute {
                    try {
                        if (sessionId == null || !sessions.containsKey(sessionId)) {
                            mainHandler.post { result.error("INVALID_SESSION", "Session not found", null) }
                            return@execute
                        }

                        if (inputs == null) {
                            mainHandler.post { result.error("INVALID_ARGUMENT", "Inputs must be a non-null map", null) }
                            return@execute
                        }
                        ...
                        mainHandler.post { result.error("INFERENCE_ERROR", e.message, e.stackTraceToString()) }
                    } catch (e: Exception) {
                        mainHandler.post { result.error("PLUGIN_ERROR", e.message, e.stackTraceToString()) }
                    }
                }
            }
            "closeSession" -> {
