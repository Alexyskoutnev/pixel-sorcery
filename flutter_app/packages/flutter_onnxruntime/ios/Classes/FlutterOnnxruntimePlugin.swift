...
public class FlutterOnnxruntimePlugin: NSObject, FlutterPlugin {
  ...
  private let inferenceQueue = DispatchQueue(label: "com.masicai.flutteronnxruntime.inference", qos: .userInitiated)
  ...
  private func handleRunInference(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    do {
      ...
      // Run inference on a background queue so we don't block the iOS main thread
      // (blocking main causes visible Flutter UI jank/freezes during long inference).
